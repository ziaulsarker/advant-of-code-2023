...
const parseLines = async lines => {
  ...
  for await (const line of lines) {
    const [game, gameRounds] = line.replace(/\s/g, "").split(":");
    const roundDetails = gameRounds.split(";")
    games.set(game, roundDetails);
  }
...
findGamesTotal("../data.txt")
.then(sunOfGames =>   console.log({sunOfGames}))
.catch(err => console.error("yoo bitch this shit anint work => ", err))